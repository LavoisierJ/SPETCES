...
os.makedirs(f'{master_path}/datasets/dataset_preliminary_work/dataset_20k/raw_dataset/', exist_ok=True)
save_path_simu = f'{master_path}/datasets/dataset_preliminary_work/dataset_20k/raw_dataset/simu_dataset_{number_of_traces}_traces_noise{noise_level}_SNR{SNR_level}.npy'
save_path_noise = f'{master_path}/datasets/dataset_preliminary_work/dataset_20k/raw_dataset/noise_dataset_{number_of_traces}_traces_noise{noise_level}_SNR{SNR_level}.npy'
    ...
    for j in range(data.shape[0]):
        ...
        if (sigma_stationnary_X < noise_level + std_noise_level) and (sigma_stationnary_X > noise_level - std_noise_level) and (sigma_stationnary_Y < noise_level + std_noise_level) and (sigma_stationnary_Y > noise_level - std_noise_level):
            ...
            if (SNR_X > SNR_level) and (SNR_Y > SNR_level):
                # We select a random position in the trace such that the signal is fully contained in a 512-sample window
                n = np.random.randint(0, 512)
                while (np.max(trace_X[n+20:n+512])!= np.max(trace_X)) and (np.max(trace_Y[n+20:n+512])!= np.max(trace_Y)) and (count_while < 1000):
                    n = np.random.randint(0, 512)
                    count_while += 1
                if count_while >= 1000:
                    count_while = 0
